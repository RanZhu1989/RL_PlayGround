...
- *Compared to off-policy algorithms:* It is hard to implement offline RL via the conventional off-policy algorithms. These off-policy algorithms are online learning implementations, which can evaluate state values which are not encountered by interacting with the environment. However, the offline RL is a different story. The values of the unseen states should be **interfered** by the agent via its inherent mechanism (data-driven, knowledge-driven, or hybrid).
